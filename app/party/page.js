...
function ApplicantView() {
  ...
  useEffect(() => {
    fetch('/api/config', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
      .then(r => r.json()).then(cfg => {
        if (cfg.party_contacts) try { setContacts(JSON.parse(cfg.party_contacts)) } catch {}
        if (cfg.party_notice) setNotice(cfg.party_notice)
      }).catch(() => {})
  }, [])
  ...
  const saveContacts = async () => {
    ...
    try {
      ...
      await saveConfig({ party_contacts: JSON.stringify(valid) })
      setContacts(valid); setEditContactOpen(false); message.success('保存成功')
  ...
  const saveNotice = async () => {
    setSaving(true)
    try {
      await saveConfig({ party_notice: editNoticeText })
      setNotice(editNoticeText); setEditNoticeOpen(false); message.success('保存成功')
    } catch { message.error('保存失败') } finally { setSaving(false) }
  }
  ...
  const handleUpload = (key) => { message.success('上传成功'); setUploaded(prev => ({ ...prev, [key]: true })) }
    ...
    <div key={0}>
      <div style={{ background: '#fffbe6', borderRadius: 10, padding: '14px 16px', marginBottom: 16, border: '1px solid #ffe58f' }}>
        ...
        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {contacts.map((c, i) => (
            <a key={i} href={c.feishuUrl || undefined} target={c.feishuUrl ? '_blank' : undefined} rel="noreferrer" onClick={e => { if (!c.feishuUrl) e.preventDefault() }}>
              <Button size="small" icon={<LinkOutlined />} style={{ borderRadius: 6, color: RED, borderColor: RED, fontSize: 12 }}>@ {c.name}（飞书）</Button>
            </a>
          ))}
          <Button type="text" size="small" onClick={() => { setEditContacts(contacts.map(c => ({ ...c }))); setEditContactOpen(true) }} style={{ fontSize: 11, color: '#98a2b3' }}>✎ 编辑</Button>
        </div>
      </div>
      ...
      {REQUIRED_MATERIALS.map(m => <MaterialItem key={m.key} material={m} uploaded={!!uploaded[m.key]} onUpload={handleUpload} />)}
    </div>,

    // 步骤1：党费凭证
    <div key={1}>
      <div style={{ background: '#fffbe6', borderRadius: 10, padding: '14px 16px', marginBottom: 16, border: '1px solid #ffe58f' }}>
        <div style={{ fontWeight: 600, color: '#101828', marginBottom: 8 }}>🪙 党费缴纳凭证说明</div>
        <div style={{ fontSize: 13, color: '#667085', lineHeight: 1.8 }}>上传最近12个月的党费缴纳凭证截图或银行流水，需清晰显示缴纳日期、金额及收款方信息。</div>
      </div>
      <MaterialItem material={{ key: 'fee', title: '党费缴纳凭证', required: true, hint: '需包含最近12个月记录，支持截图或PDF' }} uploaded={!!uploaded['fee']} onUpload={handleUpload} />
    </div>,
  ...
  return (
    <div>
      {/* 系统公告 */}
      <div style={{ background: '#fffbe6', border: '1px solid #ffe58f', borderRadius: 10, padding: '12px 16px', marginBottom: 14, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        <BellOutlined style={{ color: '#d48806', fontSize: 16, flexShrink: 0, marginTop: 2 }} />
        <div style={{ flex: 1 }}>
          <span style={{ fontWeight: 600, color: '#101828' }}>【系统公告】</span>
          <span style={{ fontSize: 13, color: '#665000', marginLeft: 6 }}>{notice}</span>
        </div>
        <Button type="text" size="small" onClick={() => { setEditNoticeText(notice); setEditNoticeOpen(true) }}
          style={{ fontSize: 12, color: '#d48806', flexShrink: 0, padding: '0 6px' }}>✎ 编辑</Button>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(16,24,40,0.06)', overflow: 'hidden' }}>
            ...
            {TRANSFER_IN_STEPS.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < TRANSFER_IN_STEPS.length - 1 ? 1 : 'none' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <div onClick={() => setCurrentStep(i)} style={{ width: 36, height: 36, borderRadius: '50%', background: i < currentStep ? '#17b26a' : i === currentStep ? RED : '#f2f4f7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, border: i === currentStep ? `2px solid ${RED}` : '2px solid transparent', boxShadow: i === currentStep ? `0 0 0 3px ${RED}20` : 'none', transition: 'all 0.2s', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '0 24px 24px' }}>
          {/* 阶段标题 */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '20px 0 16px', paddingBottom: 14, borderBottom: `1px solid #f2f4f7` }}>
            ...
            <Tag color={currentStep === TRANSFER_IN_STEPS.length - 1 ? 'success' : 'orange'} style={{ marginLeft: 'auto' }}>
              {currentStep === TRANSFER_IN_STEPS.length - 1 ? '已完成' : '进行中'}
            </Tag>
          </div>
      ...
      {/* 公告编辑弹窗 */}
      <Modal title="编辑系统公告" open={editNoticeOpen} onCancel={() => setEditNoticeOpen(false)}
        onOk={saveNotice} okText="保存" confirmLoading={saving} width={500}>
        <div style={{ padding: '8px 0' }}>
          <Input.TextArea rows={4} value={editNoticeText} onChange={e => setEditNoticeText(e.target.value)} placeholder="输入公告内容..." style={{ borderRadius: 8 }} />
        </div>
      </Modal>

      {/* 联系人编辑弹窗 */}
      <Modal title="编辑联系人" open={editContactOpen} onCancel={() => setEditContactOpen(false)}
        onOk={saveContacts} okText="保存" confirmLoading={saving} width={460}>
        <div style={{ padding: '8px 0' }}>
          <div style={{ fontSize: 12, color: '#98a2b3', marginBottom: 14 }}>
            飞书跳转链接格式：<code style={{ background: '#f5f5f5', padding: '1px 6px', borderRadius: 4 }}>https://applink.feishu.cn/client/chat/open?openId=xxx</code>
          </div>
          {editContacts.map((c, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'center' }}>
              <Input value={c.name} onChange={e => setEditContacts(p => p.map((x, j) => j === i ? { ...x, name: e.target.value } : x))} placeholder="姓名" style={{ width: 130, borderRadius: 8 }} />
              <Input value={c.feishuUrl} onChange={e => setEditContacts(p => p.map((x, j) => j === i ? { ...x, feishuUrl: e.target.value } : x))} placeholder="飞书链接（选填）" style={{ flex: 1, borderRadius: 8 }} />
              <Button type="text" danger size="small" onClick={() => setEditContacts(p => p.filter((_, j) => j !== i))}>×</Button>
            </div>
          ))}
          <Button type="dashed" onClick={() => setEditContacts(p => [...p, { name: '', feishuUrl: '' }])} style={{ width: '100%', borderRadius: 8, marginTop: 4 }}>+ 添加联系人</Button>
        </div>
      </Modal>
    </div>
  )
